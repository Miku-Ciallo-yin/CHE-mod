...
/// <summary>
/// 大厅"编辑"设置菜单的 TAHS 页签（参考 TownOfHost_Y / EHR 的 GameSettingMenuPatch）：
/// - OnEnable：克隆模板按钮和模板页签（隐藏保留，复用）
/// - Start：隐藏原版职业设置页签，重排原版按钮，创建"模组设置 / 职业设置"两个 TAHS 页签按钮和空页签
/// - ChangeTab：tabNum &gt;= 3 时拦截，隐藏原版页签、显示 TAHS 页签（原版 tabNum==2 重定向到 TAHS 职业设置）
...
public static class ModSettingsMenuPatch
{
    ...
    public static void OnEnablePostfix(GameSettingMenu __instance)
    {
        ...
        {
            TAHSPlugin.Log.LogError($"[TAHS] 设置模板创建失败: {e}");
        }
    }
    ...
    public static void StartPostfix(GameSettingMenu __instance)
    {
        try
        {
            ...
            _buttons[ModGameOptionsMenu.RolesTabIndex] = CreateTabButton(
                __instance, "TAHS_RolesButton", "职业设置",
                ModGameOptionsMenu.ButtonPosRight - row2, ModGameOptionsMenu.RolesTabIndex);

            // TAHS 空页签（内容在 ModOptionsMenuPatch 中构建）
            _tabs[ModGameOptionsMenu.ModTabIndex] = CreateTab(__instance, "TAHS_ModTab");
            _tabs[ModGameOptionsMenu.RolesTabIndex] = CreateTab(__instance, "TAHS_RolesTab");

            TAHSPlugin.Log.LogInfo("[TAHS] 模组设置 / 职业设置页签已创建");
        }
        catch (System.Exception e)
        {
            TAHSPlugin.Log.LogError($"[TAHS] 设置页签创建失败: {e}");
        }
    }
    ...
    public static bool ChangeTabPrefix(GameSettingMenu __instance, ref int tabNum)
    {
        ...
        {
            TAHSPlugin.Log.LogError($"[TAHS] 页签切换失败: {e}");
            return true;
        }
    }
