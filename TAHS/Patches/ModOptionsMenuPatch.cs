...
/// <summary>
/// TAHS 页签内容构建（参考 TOHE 的 GameOptionsMenuPatch）：
...
public static class ModOptionsMenuPatch
{
    ...
    public static bool InitializePrefix(GameOptionsMenu __instance)
    {
        ...
        {
            TAHSPlugin.Log.LogWarning($"[TAHS] 页签导航初始化跳过: {e.Message}");
        }
        return false;
    }
    ...
    public static void BuildContent(GameOptionsMenu menu)
    {
        try
        {
            ...
            {
                TAHSPlugin.Log.LogWarning($"[TAHS] 重建 ControllerSelectable 跳过: {e.Message}");
            }
        }
        catch (System.Exception e)
        {
            TAHSPlugin.Log.LogError($"[TAHS] 选项页构建失败({menu.name}): {e}");
        }
    }
...
/// <summary>
/// TAHS 选项行交互拦截（参考 TOHE：按 InstanceID 反查，return false 阻断原版逻辑）。
