...
/// - Logo 区下方加版本徽章 "TAHS vX by 米裤恰油"
/// - 底部加一行自定义按钮：关于 TAHS / GitHub / 交流群
...
public static class MainMenuPatch
{
    ...
    public static void StartPostfix(MainMenuManager __instance)
    {
        try
        {
            ...
            TAHSPlugin.Log.LogInfo("[TAHS] 主菜单定制已创建");
        }
        catch (System.Exception e)
        {
            TAHSPlugin.Log.LogError($"[TAHS] 主菜单定制失败: {e}");
        }
    }
    ...
    /// <summary>
    /// 主菜单背景：优先加载游戏目录 TAHS-DATA/background.png（放一张二次元人物图即可），
    /// 没有则用程序生成的渐变背景兜底。
    /// </summary>
    private static void SetupBackground(MainMenuManager menu)
    {
        ...
        var bg = new GameObject("TAHS_Background").AddComponent<SpriteRenderer>();
        bg.sprite = sprite;
    ...
    /// <summary>从 TAHS-DATA 加载自定义背景（优先 background.png，否则取目录里第一张 PNG）</summary>
    private static Sprite? LoadCustomBackground()
    {
        try
        {
            var dir = Path.Combine(Environment.CurrentDirectory, "TAHS-DATA");
            ...
            TAHSPlugin.Log.LogInfo($"[TAHS] 已加载自定义主菜单背景 {Path.GetFileName(path)}");
            return Sprite.Create(tex, new Rect(0, 0, tex.width, tex.height),
                new Vector2(0.5f, 0.5f), 100f);
        }
        catch (System.Exception e)
        {
            TAHSPlugin.Log.LogWarning($"[TAHS] 背景图加载失败: {e.Message}");
            return null;
        }
    }
    ...
    private static void CreateModNameLabel(MainMenuManager menu, TextMeshPro tmpTemplate)
    {
        ...
        label.text = "<color=#4FC3F7>TAHS</color>";
        label.fontSize = 4f;
    ...
    private static void CreateVersionBadge(MainMenuManager menu, TextMeshPro tmpTemplate)
    {
        ...
        badge.text = $"<color=#4FC3F7>TAHS</color> <b>v{TAHSPlugin.Version}</b> by 米裤恰油";
        badge.fontSize = 3.8f;
        // 加粗 + 黑色描边：小字号在背景上也清晰
    ...
    private static void CreateButtons(MainMenuManager menu)
    {
        ...
        CreateButton(menu, template, "关于 TAHS", basePos + new Vector3(2.4f, 0.8f, 0f),
            () => CustomPopup.Show(menu.transform, "关于 TAHS", AboutText));
