...
/// - 名单文件：游戏目录 TAHS-DATA/Moderators.txt，每行一个好友代码（FriendCode），# 为注释
...
public static class ModeratorManager
{
    ...
    public static void Init()
    {
        try
        {
            ...
            TAHSPlugin.Log.LogInfo($"[TAHS] 协管名单已加载（{_codes.Count} 人）: {FilePath}");
        }
        catch (System.Exception e)
        {
            TAHSPlugin.Log.LogError($"[TAHS] 协管名单加载失败: {e}");
        }
    }
    ...
    public static bool Add(string? friendCode)
    {
        ...
        TAHSPlugin.Log.LogInfo($"[TAHS] 新增协管: {friendCode}");
        return true;
    }
}
