...
public static class LogDumper
{
    ...
    public static void Dump()
    {
        try
        {
            ...
            var fileName = $"TAHS-Log-{timestamp}.txt";
            ...
            ShowInChat($"[TAHS] 日志已保存到桌面: {fileName}");
            foreach (var line in tail)
                ShowInChat(line);

            TAHSPlugin.Log.LogInfo($"[TAHS] 日志已导出: {dest}");
        }
        catch (System.Exception e)
        {
            TAHSPlugin.Log.LogError($"[TAHS] 日志导出失败: {e}");
            ShowInChat($"[TAHS] 日志导出失败: {e.Message}");
        }
    }
