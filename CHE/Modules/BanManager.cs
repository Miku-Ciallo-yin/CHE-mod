...
/// 名单内好友代码的玩家进房即被踢出并封禁。
/// 文件：游戏目录 CHE-DATA/BanList.txt，每行一个好友代码，# 为注释。
/// </summary>
public static class BanManager
{
    ...
    public static void Init()
    {
        try
        {
            ...
            CHEPlugin.Log.LogInfo($"[CHE] 黑名单已加载（{_codes.Count} 人）: {FilePath}");
        }
        catch (System.Exception e)
        {
            CHEPlugin.Log.LogError($"[CHE] 黑名单加载失败: {e}");
        }
    }
    ...
    public static bool Add(string? friendCode)
    {
        ...
        CHEPlugin.Log.LogInfo($"[CHE] 已加入黑名单: {friendCode}");
        return true;
    }
}
